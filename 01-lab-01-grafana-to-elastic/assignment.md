...
- **Otherwise**, bootstrap runs **`tools/seed_workshop_telemetry.py`** (bulk **`logs-workshop-default`** + **`metrics-workshop-default`**) so Discover is not empty. You can still copy the mOTLP URL from **Kibana → Add data → OpenTelemetry**, `export WORKSHOP_OTLP_ENDPOINT=…`, then run **`./scripts/start_workshop_otel.sh`**.
