...
#     uses `gh repo clone elastic/observability-migration-platform` when `gh auth status` works, else public HTTPS.
#
# Typical maintainer flow (laptop):
...
_clone_upstream_mig_to_kbn() {
  ...
  if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
    echo "==> Cloning via GitHub CLI: gh repo clone elastic/observability-migration-platform"
  ...
  echo "WARN: No MIG_TO_KBN_GIT_URL and gh not logged in; trying public HTTPS." >&2
  echo "     Fix: gh auth login   OR   export MIG_TO_KBN_GIT_URL='https://github.com/elastic/observability-migration-platform.git'" >&2
...
update_via_standalone_clone() {
  if [ ! -d "$MIG" ]; then
    ...
    echo "    git submodule add https://github.com/elastic/observability-migration-platform.git mig-to-kbn   # then commit .gitmodules" >&2
    exit 1
  fi
  if [ ! -d "$MIG/.git" ]; then
