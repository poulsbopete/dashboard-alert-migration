...
DASH_SPECS: list[tuple[str, str, str, str, str, str, str, str, str, str, str]] = [
    ...
    (
        ...
        "By entity + path",
        "sum by (entity_id, path) (rate(http_requests_total[5m]))",
        "Entity × status (instant)",
        "sum by (entity_id, status) (rate(http_requests_total[5m]))",
    ),
    (
        "03-latency-p95.json",
        "Latency p95",
    ...
    (
        ...
        "sum by (entity_id) (rate(http_requests_total[5m]))",
        "Entity × method",
        "sum by (entity_id, method) (rate(http_requests_total[5m]))",
        "Entity snapshot (instant)",
        "sum by (entity_id) (rate(http_requests_total[5m]))",
    ),
    (
        "07-post-path.json",
        "POST /v1/invoke volume",
        "**Invoke path** — focus on ``POST /v1/invoke`` vs overall POST traffic.",
        "POST /v1/invoke rps",
        "sum(rate(http_requests_total{path=\"/v1/invoke\",method=\"POST\"}[5m]))",
        "Invoke path rate",
        "sum(rate(http_requests_total{path=\"/v1/invoke\",method=\"POST\"}[5m]))",
        "All POST traffic",
        "sum(rate(http_requests_total{method=\"POST\"}[5m]))",
        "POST by path (instant)",
        "sum by (path) (rate(http_requests_total{method=\"POST\"}[5m]))",
    ),
    (
        ...
        "p90 by path",
        "histogram_quantile(0.90, sum by (le, path) (rate(http_request_duration_seconds_bucket[5m])))",
    ...
    (
        ...
        "HTTP 5xx by entity",
        "sum by (entity_id) (rate(http_requests_total{status=~\"5..\"}[5m]))",
    ...
    (
        ...
        "GC pause indicator",
        "**Go GC** — pause time sum rate as a pressure signal.",
        "GC pause sum rate",
        "sum(rate(go_gc_duration_seconds_sum[5m]))",
        "GC pause by instance",
        "sum by (instance) (rate(go_gc_duration_seconds_sum[5m]))",
        "GC count rate",
        "sum(rate(go_gc_duration_seconds_count[5m]))",
        "GC by job",
        "sum by (job) (rate(go_gc_duration_seconds_sum[5m]))",
    ),
    (
